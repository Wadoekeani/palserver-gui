...
export function registerRoutes(
  ...
  // ── Steam Workshop 模組(官方 1.0 模組系統;僅 native Windows)──
  app.get("/api/instances/:id/workshop-mods", async (req) => {
    const rec = getOr404((req.params as { id: string }).id);
    return getWorkshopModsStatus(rec, ctxOf(rec));
  });

  app.post("/api/instances/:id/workshop-mods/toggle", async (req) => {
    const rec = getOr404((req.params as { id: string }).id);
    const body = z.object({ packageName: z.string().min(1).max(200), enabled: z.boolean() }).parse(req.body);
    return setWorkshopModEnabled(rec, ctxOf(rec), body.packageName, body.enabled);
  });

  app.post("/api/instances/:id/workshop-mods/global", async (req) => {
    const rec = getOr404((req.params as { id: string }).id);
    const body = z.object({ enabled: z.boolean() }).parse(req.body);
    return setWorkshopGlobalEnabled(rec, ctxOf(rec), body.enabled);
  });
  ...
  app.post("/api/instances/:id/moderation/:action", async (req) => {
    ...
    switch (action) {
      ...
      case "ban": {
        const out = await banEverywhere(rec, ctxOf(rec), body.value, { pdActive: await isPdActive(rec), message: body.reason });
        return { ok: true, action, value: body.value, ...out };
      }
      case "unban": {
        const out = await unbanEverywhere(rec, ctxOf(rec), body.value, { pdActive: await isPdActive(rec) });
        return { ok: true, action, value: body.value, ...out };
      }
  ...
  app.put("/api/instances/:id/launch-options", async (req, reply) => {
    ...
    const loShape = Object.fromEntries(
      LAUNCH_OPTION_KEYS.map((k) => {
        ...
        if (meta.type === "text") return [k, z.string().max(1000).optional()];
